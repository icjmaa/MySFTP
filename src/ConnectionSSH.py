...
class ConnectionSSH():
	...
	def ssh_exec(args, capture_output=False):
		...
		if not pid:
			...
			if program == 'sftp':
				os.execvp(program, [program, "-o", 'PubkeyAuthentication=no', '-o', 'HostKeyAlgorithms=+ssh-rsa', '-o', 'PubkeyAcceptedAlgorithms=+ssh-rsa', port, user_host])
			elif program == 'ssh':
				os.execvp(program, [program, "-o", 'PubkeyAuthentication=no', '-o', 'HostKeyAlgorithms=+ssh-rsa', '-o', 'PubkeyAcceptedAlgorithms=+ssh-rsa', user_host, command_remote])
