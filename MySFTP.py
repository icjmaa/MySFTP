	...
	if tipo == "sftp":
		if platform.system() == "Linux":
			...
			try:
				...
				scrip_file.close()
				proceso = subprocess.Popen([mydir + "\\bin\\psftp.exe" ,"-P" , puerto, "-pw", password, "-b" , mydir + "\\script.bat", usuario + "@" + host], stdin=subprocess.PIPE,stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
				proceso.wait( 5 )
